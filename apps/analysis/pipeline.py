"""Utility functions for loading data and training the ML pipeline."""
...
from .utils import (
    CROSS_DATA_COLLECTION,
    LONG_DATA_COLLECTION,
    MERGED_DATA,
    RF_RESULT,
    Y_RESULT,
    get_collection,
...
def load_data(command, dir, file_name, collection):
    """Load a CSV file into the given MongoDB collection."""
    ...
    try:
        with open(file_path, mode="r", encoding="utf-8") as csvfile:
            ...
            if records:
                ...
                command.write_suc(
                    f"Successfully imported {file_name} documents into database."
                )
            else:
...
def train_and_store(command, dir):
    """Train the model using data from ``dir`` and store the results."""
    ...
    X = data.drop(
        ["Group", "ID", "Delay", "Subject ID", "MRI ID", "Visit", "MR Delay"], axis=1
    )
    y = data["Group"]
    ...
    preprocessor = ColumnTransformer(
        transformers=[("scale", StandardScaler(), scale), ("ohe", OneHotEncoder(), ohe)]
    )
    pipeline = Pipeline(
    ...
    get_collection(RF_RESULT).replace_one(
        {"_id": "latest"}, {"report": rf_rsult}, upsert=True
    )
    formatted_probs = y_prob.tolist() if hasattr(y_prob, "tolist") else list(y_prob)
    get_collection(Y_RESULT).replace_one(
        {"_id": "latest"}, {"y_prob": formatted_probs}, upsert=True
    )
