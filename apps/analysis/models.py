"""Django models for storing medical datasets and results."""

from django.db import models


class CrossData(models.Model):
    """Single visit demographic and MRI measurements."""

    id = models.CharField(max_length=50, primary_key=True)
    gender = models.CharField(max_length=1, choices=[("M", "Male"), ("F", "Female")])
    handedness = models.CharField(
        max_length=1, choices=[("R", "Right"), ("L", "Left"), ("A", "Ambidextrous")]
    )
    ...
    mmse = models.FloatField(
        null=True, blank=True, help_text="Mini-Mental State Examination Score"
    )
    cdr = models.FloatField(null=True, blank=True, help_text="Clinical Dementia Rating")
    etiv = models.FloatField(
        null=True, blank=True, help_text="Estimated Total Intracranial Volume"
    )
    nwbv = models.FloatField(
        null=True, blank=True, help_text="Normalized Whole Brain Volume"
    )
...
class LongData(models.Model):
    """Longitudinal data collected across multiple visits."""
