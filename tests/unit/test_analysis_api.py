...
from apps.analysis.models import RFResult, YProb, MergedData
from apps.common.repositories.analysis_repository import (
    RFResultRepository, YProbRepository, MergedDataRepository,
)
...
def test_data_endpoint(auth_client, monkeypatch):
    MergedData.objects.create(source_id='s1', subject_id='sub', group='A', visit=1, delay=0,
                              gender='M', handedness='R', age=30)
    MergedData.objects.create(source_id='s2', subject_id='sub', group='B', visit=2, delay=1,
                              gender='F', handedness='L', age=31)
...
def test_train_endpoint(auth_client, monkeypatch):
    ...
    monkeypatch.setattr('apps.analysis.views.call_command', fake_call)
